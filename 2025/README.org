...
After doing 2024 in Lua, which felt very Python but without all the nice things, I wanted
to concentrate on algorithms.

So Python it is.

* Day 1

A simple instruction-following problem. The only trick - using mod.
...
Now this was an exercise in corner case finding! First one was pure iteration over space
...
rectangle, line length is always at least 3. Took some time to figure things out properly.
...
The second one turned out too hard for my naive Python and BFS no matter what I tried. So
I reformulated it in terms the Z3 Python constraint solver. For some reason it feels like
cheating.

* Day 11

The first problem was easy to solve using BFS.

I struggled a bit to scale up the solution to the second problem until I realized (using a
graphviz graph visualization) that input.txt was a DAG, which made topological sort
possible. Having a well-defined order made counting paths trivial.

* Day 12

Well, this one was about checking if the stupid thing works first.
